...
Operator-facing verbs. Each command is a thin spec: input, files to load, files to update, what to surface. The agent reads the spec, executes against the brain, and reports back per `AGENTS.md § Operating loop`.
...
- Every command ends by surfacing 2-4 bullets per `AGENTS.md § Operating loop § 7`.
...
- Commands respect `AGENTS.md § Operating preferences § Autonomy mode`. Under `propose and wait`, drafts are presented for approval before saving.
